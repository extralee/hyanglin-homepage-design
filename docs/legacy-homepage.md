...
> ```
> 💡 **참고 (IP 직접 접속 시)**: 도메인이 아닌 `14.63.198.35` IP로 직접 접속했을 때 법무법인 등 다른 사이트가 노출되는 문제를 해결하기 위해, `httpd.conf` 최상단에 IP 주소를 ServerName으로 하고 DocumentRoot를 `/home/hr/www`로 설정하는 폴백(Fallback) VirtualHost 블록이 추가되었습니다.
